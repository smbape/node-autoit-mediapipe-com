...
Global Const $_MODEL_FILE = $MEDIAPIPE_SAMPLES_DATA_PATH & "\ptm_512_hdt_ptm_woid.tflite"
...
Global $FormGUI = GUICreate("Interactive segmenter", 1065, 617, 192, 124)

Global $InputImage = GUICtrlCreateInput($cv.samples.findFile("cats_and_dogs.jpg"), 264, 24, 449, 21)
GUICtrlSetState(-1, $GUI_DISABLE)
Global $BtnImage = GUICtrlCreateButton("Open", 723, 22, 75, 25)

Global $BtnExec = GUICtrlCreateButton("Execute", 813, 22, 75, 25)

Global $LabelBlurred = GUICtrlCreateLabel("Blurred", 231, 60, 100, 20)
GUICtrlSetFont(-1, 10, 800, 0, "MS Sans Serif")
Global $GroupBlurred = GUICtrlCreateGroup("", 20, 83, 510, 516)
Global $PicBlurred = GUICtrlCreatePic("", 25, 94, 500, 500)
GUICtrlCreateGroup("", -99, -99, 1, 1)

Global $LabelOutput = GUICtrlCreateLabel("Output", 735, 60, 148, 20)
GUICtrlSetFont(-1, 10, 800, 0, "MS Sans Serif")
Global $GroupOutput = GUICtrlCreateGroup("", 532, 83, 510, 516)
Global $PicOutput = GUICtrlCreatePic("", 537, 94, 500, 500)
GUICtrlCreateGroup("", -99, -99, 1, 1)
...
Func Main($x = 0.68, $y = 0.68)
	...
	Local Static $FG_COLOR = _OpenCV_Scalar(255, 255, 255) ; white
	...
	Local $scale = 1 / _OpenCV_resizeRatio_ControlPic($image.mat_view(), $FormGUI, $PicOutput) ; keep drawings visible after resize

	; Retrieve the masks for the segmented image
	Local $roi = $RegionOfInterest(_Mediapipe_Params("format", $RegionOfInterest_Format.KEYPOINT, _
			"keypoint", $NormalizedKeypoint($x, $y)))
	Local $category_masks = $segmenter.segment($image, $roi)

	; Foreground mask corresponds to all i pixels where category_masks[0][i] > 0.2
	Local $fg_mask = $cv.compare($category_masks[0].mat_view(), 0.2, $CV_CMP_GT)
	...
	Local $fg_image = $cv.Mat.create($image_data.size(), $CV_8UC3, $FG_COLOR)
	...
	_OpenCV_imshow_ControlPic($output_image, $FormGUI, $PicOutput)
	...
	$aSize = $blurred_image.size()

	_OpenCV_imshow_ControlPic($blurred_image, $FormGUI, $PicBlurred)
EndFunc   ;==>Main
...
Func UpdateCursor()
	...
	If $aCursor[4] <> $PicBlurred Then
		; Arrow cursor
		GUISetCursor(2, $GUI_CURSOR_NOOVERRIDE, $FormGUI)
		Return
	EndIf

	Local $aPicPos = ControlGetPos($FormGUI, "", $PicBlurred)
