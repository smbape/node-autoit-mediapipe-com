...
public static class MediapipeComInterop
{
    ...
    public static void DllOpen(string openCvWorldDll, string mediapipeComDll)
    {
        ...
        {
            throw new Win32Exception(Marshal.GetLastWin32Error(), "Failed to open autoit mediapipe com library '" + mediapipeComDll + "'");
        }
        ...
        IntPtr DllActivateManifest_addr = GetProcAddress(hMediapipeCom, "DllActivateManifest");
    ...
    public static bool DllActivateManifest(string manifest = null)
    {
        if (string.IsNullOrWhiteSpace(manifest)) {
            manifest = Environment.GetEnvironmentVariable("MEDIAPIPE_ACTCTX_MANIFEST");
        }
    ...
    public static string FindResourceDir(string rootPath = null, string buildType = null) {
        if (string.IsNullOrWhiteSpace(rootPath)) {
            rootPath = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
        }
        ...
        string file = "mediapipe\\modules\\face_detection\\face_detection_short_range_cpu.binarypb";
        string graphFile = FindFile(file, rootPath, hints.ToArray());

        if (string.IsNullOrEmpty(graphFile)) {
            return "";
        }

        return graphFile.Substring(0, graphFile.Length - file.Length - 1);
    }
}
