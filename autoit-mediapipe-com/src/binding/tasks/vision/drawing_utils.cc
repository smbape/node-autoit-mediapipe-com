...
namespace {
	...
	using namespace mediapipe::tasks::autoit::vision;
	...
	struct OptionalDrawingSpec<DrawingSpec> {
		inline static const bool has(
			...
			const tasks::components::containers::Connection connection
		) {
			return true;
		}
	...
	struct OptionalDrawingSpec<std::map<int, std::map<int, DrawingSpec>>> {
		inline static const bool has(
			...
			if (!connection_drawing_spec.count(connection.start)) {
				return false;
			}
			return connection_drawing_spec.at(connection.start).count(connection.end);
		}
		...
		inline static const DrawingSpec& get(
			...
			return connection_drawing_spec.at(connection.start).at(connection.end);
		}
	};
	...
	[[nodiscard]] absl::Status _draw_landmarks(
		cv::Mat& image,
		const std::vector<tasks::components::containers::NormalizedLandmark>& landmark_list,
		const std::vector<tasks::components::containers::Connection>& connections,
		...
		for (const auto& landmark : landmark_list) {
			...
			if (
				(landmark.visibility && *landmark.visibility < _VISIBILITY_THRESHOLD)
				|| (landmark.presence && *landmark.presence < _PRESENCE_THRESHOLD)
				) {
				continue;
			}
		...
		if (!connections.empty()) {
			auto num_landmarks = landmark_list.size();

			// Draws the connections if the start and end landmarks are both visible.
			for (const auto& connection : connections) {
				auto start_idx = connection.start;
				auto end_idx = connection.end;

				MP_ASSERT_RETURN_IF_ERROR(0 <= start_idx < num_landmarks && 0 <= end_idx < num_landmarks,
					"Landmark index is out of range (num_landmarks = " << num_landmarks << "). Invalid connection "
	...
	cv::Mat clip(const cv::Mat& a, float a_min, float a_max) {
		cv::Mat dst;
		cv::max(a, a_min, dst);
...
namespace mediapipe::tasks::autoit::vision::drawing_utils {
	absl::Status draw_detection(
		cv::Mat& image,
		const tasks::components::containers::Detection& detection,
		...
		if (detection.keypoints) {
			for (const auto& keypoint : *detection.keypoints) {
				auto keypoint_px = _normalized_to_pixel_coordinates(keypoint.x, keypoint.y, image_width, image_height);
				MP_ASSERT_RETURN_IF_ERROR(keypoint_px.get(), "Failed to get keypoint pixel coordinates");
				cv::circle(
					image,
					*keypoint_px,
					keypoint_drawing_spec.circle_radius,
					color_to_scalar(keypoint_drawing_spec.color),
		...
		// Draws bounding box.
		cv::Point rect_start_point(
			detection.bounding_box.left,
			detection.bounding_box.top
		);
		cv::Point rect_end_point(
			std::min(detection.bounding_box.right, detection.bounding_box.left + image_width - 1),
			std::min(detection.bounding_box.bottom, detection.bounding_box.top + image_height - 1)
		);

		if (rect_start_point.x != rect_end_point.x && rect_start_point.y != rect_end_point.y) {
			cv::rectangle(
				image,
				rect_start_point,
				rect_end_point,
