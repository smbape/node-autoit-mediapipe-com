...
namespace {
	...
	std::shared_ptr<FaceLandmarkerResult> _build_landmarker_result(const PacketMap& output_packets) {
...
namespace mediapipe::tasks::autoit::vision::face_landmarker {
	...
	std::shared_ptr<FaceLandmarkerGraphOptions> FaceLandmarkerOptions::to_pb2() {
		...
		if (base_options) {
			face_landmarker_options_proto->mutable_base_options()->CopyFrom(*base_options->to_pb2());
		}
	...
	std::shared_ptr<FaceLandmarker> FaceLandmarker::create_from_model_path(const std::string& model_path) {
	...
	std::shared_ptr<FaceLandmarker> FaceLandmarker::create_from_options(std::shared_ptr<FaceLandmarkerOptions> options) {
		...
		*task_info.input_streams = {
		...
		return std::make_shared<FaceLandmarker>(
	...
	std::shared_ptr<FaceLandmarkerResult> FaceLandmarker::detect_for_video(
		const Image& image,
	...
	void FaceLandmarker::detect_async(
		const Image& image,
		int64_t timestamp_ms,
		std::shared_ptr<ImageProcessingOptions> image_processing_options
	) {
