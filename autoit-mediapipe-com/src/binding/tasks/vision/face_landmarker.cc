...
namespace mediapipe::tasks::autoit::vision::face_landmarker {
	using Connection = FaceLandmarksConnections::Connection;

	const std::vector<Connection> FaceLandmarksConnections::FACE_LANDMARKS_LIPS = {
		{ 61, 146 },
		{ 146, 91 },
		{ 91, 181 },
		{ 181, 84 },
	...
	const std::vector<Connection> FaceLandmarksConnections::FACE_LANDMARKS_LEFT_EYE = {
	{ 263, 249 },
	{ 249, 390 },
	{ 390, 373 },
	{ 373, 374 },
	...
	const std::vector<Connection> FaceLandmarksConnections::FACE_LANDMARKS_LEFT_EYEBROW = {
		{ 276, 283 },
		{ 283, 282 },
		{ 282, 295 },
		{ 295, 285 },
	...
	const std::vector<Connection> FaceLandmarksConnections::FACE_LANDMARKS_LEFT_IRIS = {
		{ 474, 475 },
		{ 475, 476 },
		{ 476, 477 },
		{ 477, 474 },
	...
	const std::vector<Connection> FaceLandmarksConnections::FACE_LANDMARKS_RIGHT_EYE = {
		{ 33, 7 },
		{ 7, 163 },
		{ 163, 144 },
		{ 144, 145 },
	...
	const std::vector<Connection> FaceLandmarksConnections::FACE_LANDMARKS_RIGHT_EYEBROW = {
		{ 46, 53 },
		{ 53, 52 },
		{ 52, 65 },
		{ 65, 55 },
	...
	const std::vector<Connection> FaceLandmarksConnections::FACE_LANDMARKS_RIGHT_IRIS = {
		{ 469, 470 },
		{ 470, 471 },
		{ 471, 472 },
		{ 472, 469 },
	...
	const std::vector<Connection> FaceLandmarksConnections::FACE_LANDMARKS_FACE_OVAL = {
		{ 10, 338 },
		{ 338, 297 },
		{ 297, 332 },
		{ 332, 284 },
	...
	static std::vector<Connection> get_FACE_LANDMARKS_CONTOURS() {
		std::vector<Connection> connections;

		// preallocate memory
		connections.reserve(
			FaceLandmarksConnections::FACE_LANDMARKS_LIPS.size() +
			FaceLandmarksConnections::FACE_LANDMARKS_LEFT_EYE.size() +
			FaceLandmarksConnections::FACE_LANDMARKS_LEFT_EYEBROW.size() +
			FaceLandmarksConnections::FACE_LANDMARKS_RIGHT_EYE.size() +
			FaceLandmarksConnections::FACE_LANDMARKS_RIGHT_EYEBROW.size() +
			FaceLandmarksConnections::FACE_LANDMARKS_FACE_OVAL.size()
		);

		connections.insert(connections.end(), FaceLandmarksConnections::FACE_LANDMARKS_LIPS.begin(), FaceLandmarksConnections::FACE_LANDMARKS_LIPS.end());
		connections.insert(connections.end(), FaceLandmarksConnections::FACE_LANDMARKS_LEFT_EYE.begin(), FaceLandmarksConnections::FACE_LANDMARKS_LEFT_EYE.end());
		connections.insert(connections.end(), FaceLandmarksConnections::FACE_LANDMARKS_LEFT_EYEBROW.begin(), FaceLandmarksConnections::FACE_LANDMARKS_LEFT_EYEBROW.end());
		connections.insert(connections.end(), FaceLandmarksConnections::FACE_LANDMARKS_RIGHT_EYE.begin(), FaceLandmarksConnections::FACE_LANDMARKS_RIGHT_EYE.end());
		connections.insert(connections.end(), FaceLandmarksConnections::FACE_LANDMARKS_RIGHT_EYEBROW.begin(), FaceLandmarksConnections::FACE_LANDMARKS_RIGHT_EYEBROW.end());
		connections.insert(connections.end(), FaceLandmarksConnections::FACE_LANDMARKS_FACE_OVAL.begin(), FaceLandmarksConnections::FACE_LANDMARKS_FACE_OVAL.end());

		return connections;
	}

	const std::vector<Connection> FaceLandmarksConnections::FACE_LANDMARKS_CONTOURS = get_FACE_LANDMARKS_CONTOURS();

	const std::vector<Connection> FaceLandmarksConnections::FACE_LANDMARKS_TESSELATION = {
		{ 202, 212 },
		{ 212, 214 },
		{ 214, 202 },
		{ 83, 18 },
...
PTR_BRIDGE_IMPL(mediapipe::tasks::autoit::vision::face_landmarker::FaceLandmarkerResultRawCallback);
...
const HRESULT autoit_to(VARIANT const* const& in_val, mediapipe::tasks::autoit::vision::face_landmarker::FaceLandmarkerResultCallback& out_val) {
	return autoit_to_callback<
		mediapipe::tasks::autoit::vision::face_landmarker::FaceLandmarkerResultCallback,
...
namespace {
	...
	const auto& FACE_LANDMARKS_LIPS = FaceLandmarksConnections::FACE_LANDMARKS_LIPS;
	const auto& FACE_LANDMARKS_LEFT_EYE = FaceLandmarksConnections::FACE_LANDMARKS_LEFT_EYE;
	const auto& FACE_LANDMARKS_LEFT_EYEBROW = FaceLandmarksConnections::FACE_LANDMARKS_LEFT_EYEBROW;
	const auto& FACE_LANDMARKS_LEFT_IRIS = FaceLandmarksConnections::FACE_LANDMARKS_LEFT_IRIS;
	const auto& FACE_LANDMARKS_RIGHT_EYE = FaceLandmarksConnections::FACE_LANDMARKS_RIGHT_EYE;
	const auto& FACE_LANDMARKS_RIGHT_EYEBROW = FaceLandmarksConnections::FACE_LANDMARKS_RIGHT_EYEBROW;
	const auto& FACE_LANDMARKS_RIGHT_IRIS = FaceLandmarksConnections::FACE_LANDMARKS_RIGHT_IRIS;
	const auto& FACE_LANDMARKS_FACE_OVAL = FaceLandmarksConnections::FACE_LANDMARKS_FACE_OVAL;
	const auto& FACE_LANDMARKS_CONTOURS = FaceLandmarksConnections::FACE_LANDMARKS_CONTOURS;
	const auto& FACE_LANDMARKS_TESSELATION = FaceLandmarksConnections::FACE_LANDMARKS_TESSELATION;
	...
	const std::string _BLENDSHAPES_STREAM_NAME = "blendshapes";
	const std::string _BLENDSHAPES_TAG = "BLENDSHAPES";
	const std::string _FACE_GEOMETRY_STREAM_NAME = "face_geometry";
	const std::string _FACE_GEOMETRY_TAG = "FACE_GEOMETRY";
	const std::string _TASK_GRAPH_NAME = "mediapipe.tasks.vision.face_landmarker.FaceLandmarkerGraph";
	const int64_t _MICRO_SECONDS_PER_MILLISECOND = 1000;

	[[nodiscard]] absl::StatusOr<std::shared_ptr<FaceLandmarkerResult>> _build_landmarker_result(const PacketMap& output_packets) {
		...
		MP_PACKET_ASSIGN_OR_RETURN(const auto& face_landmarks_proto_list, std::vector<NormalizedLandmarkList>, output_packets.at(_NORM_LANDMARKS_STREAM_NAME));
		...
		if (output_packets.count(_BLENDSHAPES_STREAM_NAME)) {
			MP_PACKET_ASSIGN_OR_RETURN(const auto& face_blendshapes_proto_list, std::vector<ClassificationList>, output_packets.at(_BLENDSHAPES_STREAM_NAME));
		...
		if (output_packets.count(_FACE_GEOMETRY_STREAM_NAME)) {
			MP_PACKET_ASSIGN_OR_RETURN(const auto& facial_transformation_matrixes_proto_list, std::vector<FaceGeometry>, output_packets.at(_FACE_GEOMETRY_STREAM_NAME));
...
namespace mediapipe::tasks::autoit::vision::face_landmarker {
	...
	absl::StatusOr<std::shared_ptr<FaceLandmarkerGraphOptions>> FaceLandmarkerOptions::to_pb2() const {
		auto face_landmarker_options_proto = std::make_shared<FaceLandmarkerGraphOptions>();

		// Initialize the face landmarker options from base options.
		if (base_options) {
			MP_ASSIGN_OR_RETURN(auto base_options_proto, base_options->to_pb2());
			face_landmarker_options_proto->mutable_base_options()->CopyFrom(*base_options_proto);
		}
		face_landmarker_options_proto->mutable_base_options()->set_use_stream_mode(running_mode != VisionTaskRunningMode::IMAGE);

		// Configure face detector options.
		face_landmarker_options_proto->mutable_face_detector_graph_options()->set_num_faces(num_faces);
		face_landmarker_options_proto->mutable_face_detector_graph_options()->set_min_detection_confidence(min_face_detection_confidence);

		// Configure face landmark detector options.
		face_landmarker_options_proto->set_min_tracking_confidence(min_tracking_confidence);
		face_landmarker_options_proto->mutable_face_landmarks_detector_graph_options()->set_min_detection_confidence(min_face_presence_confidence);
	...
	absl::StatusOr<std::shared_ptr<FaceLandmarker>> FaceLandmarker::create_from_options(std::shared_ptr<FaceLandmarkerOptions> options) {
		...
		if (options->result_callback) {
			packet_callback = [options](const PacketMap& output_packets) {
				...
				MP_ASSIGN_OR_THROW(auto face_landmarker_result, _build_landmarker_result(output_packets)); // There is no other choice than throw in a callback to stop the execution
	...
	absl::StatusOr<std::shared_ptr<FaceLandmarkerResult>> FaceLandmarker::detect(
		const Image& image,
	...
	absl::StatusOr<std::shared_ptr<FaceLandmarkerResult>> FaceLandmarker::detect_for_video(
		const Image& image,
	...
	absl::Status FaceLandmarker::detect_async(
		const Image& image,
		int64_t timestamp_ms,
		std::shared_ptr<ImageProcessingOptions> image_processing_options
	) {
