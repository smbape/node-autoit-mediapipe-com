...
namespace mediapipe::tasks::autoit::vision::face_detector {
	...
	struct CV_EXPORTS_W_SIMPLE FaceDetectorOptions {
		...
		CV_WRAP std::shared_ptr<mediapipe::tasks::vision::face_detector::proto::FaceDetectorGraphOptions> to_pb2();
	...
	public:
		...
		CV_WRAP static std::shared_ptr<FaceDetector> create_from_model_path(const std::string& model_path);
		CV_WRAP static std::shared_ptr<FaceDetector> create_from_options(std::shared_ptr<FaceDetectorOptions> options);
		CV_WRAP std::shared_ptr<FaceDetectorResult> detect(
			const Image& image,
		...
		CV_WRAP std::shared_ptr<FaceDetectorResult> detect_for_video(
			const Image& image,
