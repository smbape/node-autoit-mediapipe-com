#include "binding/tasks/vision/drawing_styles.h"

namespace {
	using namespace mediapipe::tasks::autoit::vision::drawing_styles;
	using namespace mediapipe::tasks::autoit::vision::drawing_utils;
	using namespace mediapipe::tasks::vision::face_landmarker;
	using namespace mediapipe::tasks::vision::hand_landmarker;
	using namespace mediapipe::tasks::vision::pose_landmarker;
	// using namespace mediapipe::tasks::components::containers;

	DrawingSpec GetDrawingSpectWithScale(const DrawingSpec& drawing_spec, float scale) {
		if (scale == 1.0) {
			return drawing_spec;
		}
		DrawingSpec result(drawing_spec);
		result.thickness *= scale;
		result.circle_radius *= scale;
		return result;
	}

	const int _RADIUS = 5;
	const DrawingColor _RED = { 48, 48, 255 };
	const DrawingColor _GREEN = { 48, 255, 48 };
	const DrawingColor _BLUE = { 192, 101, 21 };
	const DrawingColor _YELLOW = { 0, 204, 255 };
	const DrawingColor _GRAY = { 128, 128, 128 };
	const DrawingColor _PURPLE = { 128, 64, 128 };
	const DrawingColor _PEACH = { 180, 229, 255 };
	const DrawingColor _WHITE = { 224, 224, 224 };
	const DrawingColor _CYAN = { 192, 255, 48 };
	const DrawingColor _MAGENTA = { 192, 48, 255 };

	// Hands
	const int _THICKNESS_WRIST_MCP = 3;
	const int _THICKNESS_FINGER = 2;
	const int _THICKNESS_DOT = -1;

	// Hand landmarks
	const std::vector<HandLandmark> _PALM_LANDMARKS = {
		HandLandmark::WRIST,
		HandLandmark::THUMB_CMC,
		HandLandmark::INDEX_FINGER_MCP,
		HandLandmark::MIDDLE_FINGER_MCP,
		HandLandmark::RING_FINGER_MCP,
		HandLandmark::PINKY_MCP,
	};
	const std::vector<HandLandmark> _THUMB_LANDMARKS = {
		HandLandmark::THUMB_MCP,
		HandLandmark::THUMB_IP,
		HandLandmark::THUMB_TIP,
	};
	const std::vector<HandLandmark> _INDEX_FINGER_LANDMARKS = {
		HandLandmark::INDEX_FINGER_PIP,
		HandLandmark::INDEX_FINGER_DIP,
		HandLandmark::INDEX_FINGER_TIP,
	};
	const std::vector<HandLandmark> _MIDDLE_FINGER_LANDMARKS = {
		HandLandmark::MIDDLE_FINGER_PIP,
		HandLandmark::MIDDLE_FINGER_DIP,
		HandLandmark::MIDDLE_FINGER_TIP,
	};
	const std::vector<HandLandmark> _RING_FINGER_LANDMARKS = {
		HandLandmark::RING_FINGER_PIP,
		HandLandmark::RING_FINGER_DIP,
		HandLandmark::RING_FINGER_TIP,
	};
	const std::vector<HandLandmark> _PINKY_FINGER_LANDMARKS = {
		HandLandmark::PINKY_PIP,
		HandLandmark::PINKY_DIP,
		HandLandmark::PINKY_TIP,
	};

	const std::vector<std::tuple<std::vector<HandLandmark>, DrawingSpec>> _HAND_LANDMARK_STYLE = {
		{_PALM_LANDMARKS, DrawingSpec(_RED, _THICKNESS_DOT, _RADIUS)},
		{_THUMB_LANDMARKS, DrawingSpec(_PEACH, _THICKNESS_DOT, _RADIUS)},
		{_INDEX_FINGER_LANDMARKS, DrawingSpec(_PURPLE, _THICKNESS_DOT, _RADIUS)},
		{_MIDDLE_FINGER_LANDMARKS, DrawingSpec(_YELLOW, _THICKNESS_DOT, _RADIUS)},
		{_RING_FINGER_LANDMARKS, DrawingSpec(_GREEN, _THICKNESS_DOT, _RADIUS)},
		{_PINKY_FINGER_LANDMARKS, DrawingSpec(_BLUE, _THICKNESS_DOT, _RADIUS)},
	};

	// Hands connections
	const std::vector<std::tuple<std::vector<HandLandmarksConnections::Connection>, DrawingSpec>>& _HAND_CONNECTION_STYLE() {
		static const std::vector<std::tuple<std::vector<HandLandmarksConnections::Connection>, DrawingSpec>> _HAND_CONNECTION_STYLE = {
			{HandLandmarksConnections::HAND_PALM_CONNECTIONS(), DrawingSpec(_GRAY, _THICKNESS_WRIST_MCP)},
			{HandLandmarksConnections::HAND_THUMB_CONNECTIONS(), DrawingSpec(_PEACH, _THICKNESS_FINGER)},
			{HandLandmarksConnections::HAND_INDEX_FINGER_CONNECTIONS(), DrawingSpec(_PURPLE, _THICKNESS_FINGER)},
			{HandLandmarksConnections::HAND_MIDDLE_FINGER_CONNECTIONS(), DrawingSpec(_YELLOW, _THICKNESS_FINGER)},
			{HandLandmarksConnections::HAND_RING_FINGER_CONNECTIONS(), DrawingSpec(_GREEN, _THICKNESS_FINGER)},
			{HandLandmarksConnections::HAND_PINKY_FINGER_CONNECTIONS(), DrawingSpec(_BLUE, _THICKNESS_FINGER)},
		};

		return _HAND_CONNECTION_STYLE;
	}

	// FaceMesh connections
	const int _THICKNESS_TESSELATION = 1;
	const int _THICKNESS_CONTOURS = 2;
	const std::vector<std::tuple<std::vector<FaceLandmarksConnections::Connection>, DrawingSpec>>& _FACE_LANDMARKER_CONTOURS_CONNECTION_STYLE() {
		static const std::vector<std::tuple<std::vector<FaceLandmarksConnections::Connection>, DrawingSpec>> _FACE_LANDMARKER_CONTOURS_CONNECTION_STYLE = {
			{FaceLandmarksConnections::FACE_LANDMARKS_LIPS(), DrawingSpec(_WHITE, _THICKNESS_CONTOURS)},
			{FaceLandmarksConnections::FACE_LANDMARKS_LEFT_EYE(), DrawingSpec(_GREEN, _THICKNESS_CONTOURS)},
			{FaceLandmarksConnections::FACE_LANDMARKS_LEFT_EYEBROW(), DrawingSpec(_GREEN, _THICKNESS_CONTOURS)},
			{FaceLandmarksConnections::FACE_LANDMARKS_RIGHT_EYE(), DrawingSpec(_RED, _THICKNESS_CONTOURS)},
			{FaceLandmarksConnections::FACE_LANDMARKS_RIGHT_EYEBROW(), DrawingSpec(_RED, _THICKNESS_CONTOURS)},
			{FaceLandmarksConnections::FACE_LANDMARKS_FACE_OVAL(), DrawingSpec(_WHITE, _THICKNESS_CONTOURS)},
		};

		return _FACE_LANDMARKER_CONTOURS_CONNECTION_STYLE;
	}

	const std::vector<std::tuple<std::vector<FaceLandmarksConnections::Connection>, DrawingSpec>>& _FACE_LANDMARKER_CONTOURS_CONNECTION_STYLE_1() {
		static const std::vector<std::tuple<std::vector<FaceLandmarksConnections::Connection>, DrawingSpec>> _FACE_LANDMARKER_CONTOURS_CONNECTION_STYLE_1 = {
			{FaceLandmarksConnections::FACE_LANDMARKS_LIPS(), DrawingSpec(_BLUE, _THICKNESS_CONTOURS)},
			{FaceLandmarksConnections::FACE_LANDMARKS_LEFT_EYE(), DrawingSpec(_CYAN, _THICKNESS_CONTOURS)},
			{FaceLandmarksConnections::FACE_LANDMARKS_LEFT_EYEBROW(), DrawingSpec(_GREEN, _THICKNESS_CONTOURS)},
			{FaceLandmarksConnections::FACE_LANDMARKS_RIGHT_EYE(), DrawingSpec(_MAGENTA, _THICKNESS_CONTOURS)},
			{FaceLandmarksConnections::FACE_LANDMARKS_RIGHT_EYEBROW(), DrawingSpec(_RED, _THICKNESS_CONTOURS)},
			{FaceLandmarksConnections::FACE_LANDMARKS_FACE_OVAL(), DrawingSpec(_WHITE, _THICKNESS_CONTOURS)},
			{FaceLandmarksConnections::FACE_LANDMARKS_NOSE(), DrawingSpec(_YELLOW, _THICKNESS_CONTOURS)},
		};

		return _FACE_LANDMARKER_CONTOURS_CONNECTION_STYLE_1;
	}

	// Pose
	const int _THICKNESS_POSE_LANDMARKS = 2;
	const std::vector<PoseLandmark> _POSE_LANDMARKS_LEFT = {
		PoseLandmark::LEFT_EYE_INNER,
		PoseLandmark::LEFT_EYE,
		PoseLandmark::LEFT_EYE_OUTER,
		PoseLandmark::LEFT_EAR,
		PoseLandmark::MOUTH_LEFT,
		PoseLandmark::LEFT_SHOULDER,
		PoseLandmark::LEFT_ELBOW,
		PoseLandmark::LEFT_WRIST,
		PoseLandmark::LEFT_PINKY,
		PoseLandmark::LEFT_INDEX,
		PoseLandmark::LEFT_THUMB,
		PoseLandmark::LEFT_HIP,
		PoseLandmark::LEFT_KNEE,
		PoseLandmark::LEFT_ANKLE,
		PoseLandmark::LEFT_HEEL,
		PoseLandmark::LEFT_FOOT_INDEX
	};

	const std::vector<PoseLandmark> _POSE_LANDMARKS_RIGHT = {
		PoseLandmark::RIGHT_EYE_INNER,
		PoseLandmark::RIGHT_EYE,
		PoseLandmark::RIGHT_EYE_OUTER,
		PoseLandmark::RIGHT_EAR,
		PoseLandmark::MOUTH_RIGHT,
		PoseLandmark::RIGHT_SHOULDER,
		PoseLandmark::RIGHT_ELBOW,
		PoseLandmark::RIGHT_WRIST,
		PoseLandmark::RIGHT_PINKY,
		PoseLandmark::RIGHT_INDEX,
		PoseLandmark::RIGHT_THUMB,
		PoseLandmark::RIGHT_HIP,
		PoseLandmark::RIGHT_KNEE,
		PoseLandmark::RIGHT_ANKLE,
		PoseLandmark::RIGHT_HEEL,
		PoseLandmark::RIGHT_FOOT_INDEX
	};
}

namespace mediapipe::tasks::autoit::vision::drawing_styles {
	std::map<int, DrawingSpec> get_default_hand_landmarks_style(float scale) {
		std::map<int, DrawingSpec> hand_landmark_style;
		for (const auto& [k, v] : _HAND_LANDMARK_STYLE) {
			DrawingSpec drawing_spec = GetDrawingSpectWithScale(v, scale);
			for (const auto& landmark : k) {
				hand_landmark_style.insert_or_assign(static_cast<int>(landmark), drawing_spec);
			}
		}
		return hand_landmark_style;
	}

	std::map<int, std::map<int, DrawingSpec>> get_default_hand_connections_style(float scale) {
		std::map<int, std::map<int, DrawingSpec>> hand_connection_style;
		for (const auto& [connections, style] : _HAND_CONNECTION_STYLE()) {
			DrawingSpec drawing_spec = GetDrawingSpectWithScale(style, scale);
			for (const auto& [start, end] : connections) {
				if (!hand_connection_style.count(start)) {
					hand_connection_style.insert_or_assign(start, std::map<int, DrawingSpec>());
				}
				hand_connection_style.at(start).insert_or_assign(end, drawing_spec);
			}
		}
		return hand_connection_style;
	}

	std::map<int, std::map<int, DrawingSpec>> get_default_face_mesh_contours_style(int i, float scale) {
		std::map<int, std::map<int, DrawingSpec>> face_mesh_contours_connection_style;
		const auto& connections_style_map = i == 1 ? _FACE_LANDMARKER_CONTOURS_CONNECTION_STYLE_1() : _FACE_LANDMARKER_CONTOURS_CONNECTION_STYLE();
		for (const auto& [connections, style] : connections_style_map) {
			DrawingSpec drawing_spec = GetDrawingSpectWithScale(style, scale);
			for (const auto& [start, end] : connections) {
				if (!face_mesh_contours_connection_style.count(start)) {
					face_mesh_contours_connection_style.insert_or_assign(start, std::map<int, DrawingSpec>());
				}
				face_mesh_contours_connection_style.at(start).insert_or_assign(end, drawing_spec);
			}
		}
		return face_mesh_contours_connection_style;
	}

	DrawingSpec get_default_face_mesh_tesselation_style(float scale) {
		return DrawingSpec(_GRAY, _THICKNESS_TESSELATION * scale);
	}

	std::map<int, std::map<int, DrawingSpec>> get_default_face_mesh_iris_connections_style(float scale) {
		std::map<int, std::map<int, DrawingSpec>> face_mesh_iris_connections_style;

		DrawingSpec left_spec(_GREEN, _THICKNESS_CONTOURS * scale);
		for (const auto& [start, end] : FaceLandmarksConnections::FACE_LANDMARKS_LEFT_IRIS()) {
			if (!face_mesh_iris_connections_style.count(start)) {
				face_mesh_iris_connections_style.insert_or_assign(start, std::map<int, DrawingSpec>());
			}
			face_mesh_iris_connections_style.at(start).insert_or_assign(end, left_spec);
		}

		DrawingSpec right_spec(_RED, _THICKNESS_CONTOURS * scale);
		for (const auto& [start, end] : FaceLandmarksConnections::FACE_LANDMARKS_RIGHT_IRIS()) {
			if (!face_mesh_iris_connections_style.count(start)) {
				face_mesh_iris_connections_style.insert_or_assign(start, std::map<int, DrawingSpec>());
			}
			face_mesh_iris_connections_style.at(start).insert_or_assign(end, right_spec);
		}

		return face_mesh_iris_connections_style;
	}

	std::map<int, DrawingSpec> get_default_pose_landmarks_style(float scale) {
		std::map<int, DrawingSpec> pose_landmark_style;

		DrawingSpec left_spec({ 0, 138, 255 }, _THICKNESS_POSE_LANDMARKS * scale);
		for (const auto& landmark : _POSE_LANDMARKS_LEFT) {
			pose_landmark_style.insert_or_assign(static_cast<int>(landmark), left_spec);
		}

		DrawingSpec right_spec({ 231, 217, 0 }, _THICKNESS_POSE_LANDMARKS * scale);
		for (const auto& landmark : _POSE_LANDMARKS_RIGHT) {
			pose_landmark_style.insert_or_assign(static_cast<int>(landmark), right_spec);
		}

		pose_landmark_style.insert_or_assign(static_cast<int>(PoseLandmark::NOSE), DrawingSpec(_WHITE, _THICKNESS_POSE_LANDMARKS * scale));

		return pose_landmark_style;
	}

}
