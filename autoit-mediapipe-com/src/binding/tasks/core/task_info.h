		...
		namespace autoit {
			namespace core {
				struct CV_EXPORTS_W_SIMPLE TaskInfo {
					...
					CV_WRAP TaskInfo(
						const std::shared_ptr<BaseOptions>& task_options,
						const std::string& task_graph = "",
						const std::vector<std::string>& input_streams = std::vector<std::string>(),
						const std::vector<std::string>& output_streams = std::vector<std::string>()
					) :
						...
						input_streams(input_streams),
						output_streams(output_streams),
						task_options(task_options->to_pb2())
					{}
					...
					CV_PROP_RW std::shared_ptr<mediapipe::tasks::core::proto::BaseOptions> task_options;
				};
			}
		}
	}
