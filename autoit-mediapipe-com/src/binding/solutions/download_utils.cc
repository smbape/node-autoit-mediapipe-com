...
namespace mediapipe::autoit::solutions::download_utils {
	absl::Status download_oss_model(const std::string& model_path) {
		...
		return download(model_url, model_abspath.string());
	}

	absl::Status download(const std::string& url, const std::string& file) {
