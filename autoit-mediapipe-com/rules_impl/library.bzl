...
def _cc_compile(srcs, copts, ctx, additional_inputs = []):
    ...
    return cc_common.compile(
        ...
        additional_inputs = ctx.files.additional_compiler_inputs + additional_inputs,
    )
...
def add_com_library(name, intdir, compilation_mode):
    ...
    _add_com_library(
        ...
        copts = ["/we4834", "/Zc:__cplusplus"] + select({
            ":windows-opt-dbg": [
                "/Z7",
            ],
            "//conditions:default": [],
