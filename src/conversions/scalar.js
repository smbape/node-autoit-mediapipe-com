...
module.exports = (header = [], impl = [], options = {}) => {
    ...
    header.push(`
        ...
        const bool is_assignable_from(cv::Scalar_<_Tp>& out_val, VARIANT const* in_val, bool is_optional) {
            ${ optional.check.join(`\n${ " ".repeat(12) }`) }

            // a number can be a Scalar
        ...
        template<typename _Tp>
        inline const bool is_assignable_from(AUTOIT_PTR<cv::Scalar_<_Tp>>& out_val, VARIANT const* in_val, bool is_optional) {
            static cv::Scalar_<_Tp> tmp;
            return is_assignable_from(tmp, in_val, is_optional);
        }

        template<typename _Tp>
        const HRESULT autoit_to(VARIANT const* in_val, cv::Scalar_<_Tp>& out_val) {
        ...
        template<typename _Tp>
        inline const HRESULT autoit_to(VARIANT const* in_val, AUTOIT_PTR<cv::Scalar_<_Tp>>& out_val) {
            out_val = ${ options.make_shared }<cv::Scalar_<_Tp>>();
            return autoit_to(in_val, *out_val.get());
        }

        template<typename _Tp>
        const HRESULT autoit_from(cv::Scalar_<_Tp> const& in_val, VARIANT*& out_val) {
