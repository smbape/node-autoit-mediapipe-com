...
Global Const $image_frame = _Mediapipe_ObjCreate("mediapipe.tasks.autoit.vision.core.image_frame")
_AssertIsObj($image_frame, "Failed to load mediapipe.tasks.autoit.vision.core.image_frame")
...
EndFunc   ;==>ImageFrameTest

Func test_create_image_frame_from_gray_cv_mat()
	ConsoleWrite('"' & @ScriptFullPath & '" @@ Debug(' & @ScriptLineNumber & ') : test_create_image_frame_from_gray_cv_mat' & @CRLF) ;### Debug Console
	...
	$gray8_image_frame = $ImageFrame($MEDIAPIPE_IMAGE_FORMAT_GRAY8, $cv_image)
	$gray8_image_frame_mat = $gray8_image_frame.mat_view()
	_AssertMatEqual($cv_image, $gray8_image_frame_mat)
	...
	$gray8_image_frame = $ImageFrame($cv_image)
...
Func test_create_image_frame_from_rgb_cv_mat()
	ConsoleWrite('"' & @ScriptFullPath & '" @@ Debug(' & @ScriptLineNumber & ') : test_create_image_frame_from_rgb_cv_mat' & @CRLF) ;### Debug Console
	...
	; specify the image format
	$rgb_image_frame = $ImageFrame($MEDIAPIPE_IMAGE_FORMAT_SRGB, $cv_image)
	$rgb_image_frame_mat = $rgb_image_frame.mat_view()
	_AssertMatEqual($cv_image, $rgb_image_frame_mat)
	...
	; infer format from mat
	$rgb_image_frame = $ImageFrame($cv_image)
...
Func test_create_image_frame_from_rgb48_cv_mat()
	ConsoleWrite('"' & @ScriptFullPath & '" @@ Debug(' & @ScriptLineNumber & ') : test_create_image_frame_from_rgb48_cv_mat' & @CRLF) ;### Debug Console
	...
	; specify the image format
	$rgb48_image_frame = $ImageFrame($MEDIAPIPE_IMAGE_FORMAT_SRGB48, $cv_image)
	$rgb48_image_frame_mat = $rgb48_image_frame.mat_view()
	_AssertMatEqual($cv_image, $rgb48_image_frame_mat)
	...
	; infer format from mat
	$rgb48_image_frame = $ImageFrame($cv_image)
...
Func test_image_frame_mat_view_with_contiguous_data()
	ConsoleWrite('"' & @ScriptFullPath & '" @@ Debug(' & @ScriptLineNumber & ') : test_image_frame_mat_view_with_contiguous_data' & @CRLF) ;### Debug Console

	Local $w = 640
	Local $h = 480
	Local $cv_image = _RandomImage($w, $h, $CV_8UC3, 0, 2 ^ 8)

	Local $rgb_image_frame = $ImageFrame($MEDIAPIPE_IMAGE_FORMAT_SRGB, $cv_image)
...
Func test_image_frame_numpy_view_with_non_contiguous_data()
	ConsoleWrite('"' & @ScriptFullPath & '" @@ Debug(' & @ScriptLineNumber & ') : test_image_frame_numpy_view_with_non_contiguous_data' & @CRLF) ;### Debug Console
