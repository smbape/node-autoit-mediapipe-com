...
;~ Sources:
;~     https://github.com/google-ai-edge/mediapipe/blob/v0.10.35/mediapipe/tasks/python/test/vision/pose_landmarker_test.py
...
Func PoseLandmarkerTest()
	PoseLandmarkerTest_setUp()
...
EndFunc   ;==>PoseLandmarkerTest

Func PoseLandmarkerTest_setUp()
...
Func test_detect_for_video($image_path, $rotation, $output_segmentation_masks, $expected_result)
	...
	For $timestamp = 0 To (300 - 30) Step 30
		...
		If UBound($result.pose_landmarks) > 0 Then
			_expect_pose_landmarker_results_correct( _
					$result, _
					$expected_result, _
					$output_segmentation_masks, _
