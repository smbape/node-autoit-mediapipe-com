...
Global Const $_ImageFormat = $image_module.ImageFormat
...
Global Const $_MASK_SIMILARITY_THRESHOLD = 0.96
...
Func _similar_to_float_mask($actual_mask, $expected_mask, $similarity_threshold)
	_AssertIsNotNone($actual_mask, "Result mask used for comparison was None")
...
InteractiveSegmenterTest()


Func InteractiveSegmenterTest()
	InteractiveSegmenterTest_setUp()
	...
	test_segment_succeeds_with_category_mask( _
			$FILE_NAME, _
			$MEDIAPIPE_TASKS_VISION_INTERACTIVE_SEGMENTER_REGION_OF_INTEREST_FORMAT_KEYPOINT, _
...
EndFunc   ;==>InteractiveSegmenterTest

Func InteractiveSegmenterTest_setUp()
...
Func test_segment_succeeds_with_category_mask( _
	...
	ConsoleWrite('"' & @ScriptFullPath & '" @@ Debug(' & @ScriptLineNumber & ') : test_segment_succeeds_with_category_mask' & @CRLF) ;### Debug Console
	...
	_AssertIsNotNone($category_mask, "Category mask was None")
...
Func test_segment_succeeds_with_confidence_mask($model_file_type, $roi_format, $keypoint, $output_mask, $similarity_threshold)
	ConsoleWrite('"' & @ScriptFullPath & '" @@ Debug(' & @ScriptLineNumber & ') : test_segment_succeeds_with_confidence_mask' & @CRLF) ;### Debug Console
...
Func test_segment_succeeds_with_rotation()
	ConsoleWrite('"' & @ScriptFullPath & '" @@ Debug(' & @ScriptLineNumber & ') : test_segment_succeeds_with_rotation' & @CRLF) ;### Debug Console

	; Creates segmenter.
	Local $base_options = $_BaseOptions(_Mediapipe_Params("model_asset_path", $model_path))
	Local $roi = $_RegionOfInterest(_Mediapipe_Params( _
			"format", $MEDIAPIPE_TASKS_VISION_INTERACTIVE_SEGMENTER_REGION_OF_INTEREST_FORMAT_KEYPOINT, _
