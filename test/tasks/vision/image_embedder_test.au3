...
Func ImageEmbedderTest()
	ImageEmbedderTest_setUp()
...
EndFunc   ;==>ImageEmbedderTest

Func ImageEmbedderTest_setUp()
...
Func test_embed($l2_normalize, $quantize, $with_roi, $model_file_type, $expected_similarity, $expected_size, $expected_first_values)
	...
	If $with_roi Then
		...
		Local $roi = $_RectF(_Mediapipe_Params("left", 0.0, "top", 0.0, "right", 0.833333, "bottom", 1.0))
...
Func test_embed_for_video()
	ConsoleWrite('"' & @ScriptFullPath & '" @@ Debug(' & @ScriptLineNumber & ') : test_embed_for_video' & @CRLF) ;### Debug Console
...
Func test_embed_for_video_succeeds_with_region_of_interest()
	ConsoleWrite('"' & @ScriptFullPath & '" @@ Debug(' & @ScriptLineNumber & ') : test_embed_for_video_succeeds_with_region_of_interest' & @CRLF) ;### Debug Console
