...
Func _get_expected_face_landmarks($file_path)
	...
	Local $face_landmarks_results[1][UBound($face_landmarks)]

	For $i = 0 To UBound($face_landmarks) - 1
		$face_landmarks_results[0][$i] = $face_landmarks[$i]
	Next
...
Func _get_expected_face_blendshapes($file_path)
	...
	Local $face_blendshapes_results[1][UBound($face_blendshapes_categories)]

	For $i = 0 To UBound($face_blendshapes_categories) - 1
		$face_blendshapes_results[0][$i] = $face_blendshapes_categories[$i]
	Next
...
Func FaceLandmarkerTest()
	FaceLandmarkerTest_setUp()
...
EndFunc   ;==>FaceLandmarkerTest


Func FaceLandmarkerTest_setUp()
